...
class Farmer(models.Model):
    ...
    assigned_id = models.CharField(max_length=20, unique=True, editable=False, blank=True)
    ...
    def save(self, *args, **kwargs):
        while not self.assigned_id:
            ...
            newslug = ''.join(str(gen) for gen in generated)

            if not self.__class__.objects.filter(assigned_id=newslug).exists():
                self.assigned_id = 'FRMR-' + newslug
...
class UsersAreaInfo(models.Model):
    ...
    sketch_plan = models.FileField(upload_to='area_info/documents_sp/%Y%m%d', max_length=254, blank=True)
    map = models.FileField(upload_to='area_info/documents_map/%Y%m%d', max_length=254, blank=True)
    google_earth = models.FileField(upload_to='area_info/documents_ge/%Y%m%d', max_length=254, blank=True)
    area_coordinates = models.FileField(upload_to='area_info/documents_coordinates/%Y%m%d', max_length=254, blank=True)
